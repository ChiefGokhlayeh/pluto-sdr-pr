...
\end{equation}
Hierbei gilt, dass jede Gruppe eine Nummer $N_{ID}^{(1)}$ zur Identifikation im Bereich von 0 bis 167 besitzt. Die Identifikationsnummer $N_{ID}^{(2)}$ der physikalischen Ebene in der physikalischen Gruppe im Bereich von 0 bis 2.
...
Der Framestrukturtyp 1, der für Vollduplex und Halbdublex FDD gilt, hat eine Länge von \SI{10}{\milli\second} und besteht aus 10 Subframes, die jeweils eine Länge von \SI{1}{\milli\second} haben und von 0 bis 9 durchnummeriert sind. Bei Subframes, die die Frequenz $\Delta f=$ \SI{2.5}{\kilo\hertz}, $\Delta f=$\SI{7.5}{\kilo\hertz} oder $\Delta f=$ \SI{15}{\kilo\hertz} besitzen wird ein Subframe in zwei Slots unterteilt, wo die länge jedes Slots \SI{0.5}{\milli\second} beträgt. Bei einer Subframefrequenz von $\Delta f=$ \SI{1.25}{\kilo\hertz} besteht der Subframe aus keinen Slots und seine Länge beträgt \SI{1}{\milli\second}. Wenn die Übertragungsfrequenz bei $\Delta f=$ \SI{0.37}{\kilo\hertz} liegt, besitzt ein Slot eine Länge von \SI{3}{\milli\second}. Eine Periode ist \SI{40}{\milli\second} lang und besteht dem nach aus 12 Slots, die von 0 bis 13 durchnummeriert sind. Die Übertragung beginnt somit bei jedem vierten Frame. Außerdem stehen für FDD 10 Subframes, 20 Slots oder bis 60 Subslots zur Verfügung. In Abbildung~\ref{fig:LTE_frame} ist zum besseren Verständnis der Frame mit Subframes und Slots dargestellt.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node [draw, thick, shape=rectangle, minimum width=2cm, minimum height=0.8cm, anchor=center] at (6,0) {$...$};
