...
\begin{table}
    \centering
    \begin{tabular}[h]{rl}
        Empfänger      & \SIrange{325}{3800}{\mega\hertz} Truner-Frequenz, \SIrange{0.2}{20}{\mega\hertz} Bandbreite \\
        Sender         & \SIrange{325}{3800}{\mega\hertz} Truner-Frequenz, \SIrange{0.2}{20}{\mega\hertz} Bandbreite \\
...
\begin{table}
    \centering
    \begin{tabular}[h]{rl}
        Antenne         & SKT SL43-01 UHF 43                                              \\ % chktex 8
        Antennengewinn  & 11..13 dB                                                       \\
        Frequenzbereich & \SIrange{470}{862}{\mega\hertz}                                 \\
        Halbwertsbreite & horiz. \SIrange{30}{40}{\degree}/ver. \SIrange{35}{50}{\degree} \\
    \end{tabular}
    \caption{Daten der SKT SL43-01 UHF 43 Antenne.\label{table:antenne}} % chktex 8
\end{table}

\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{images/antenne.png}
    \quelle{\cite{ANTENNE2021}}
    \caption{SKT SL43-01 UHF 43 Antenne\label{fig:antenne}} % chktex 8
\end{figure}

\section{Signal}

Das betrachtete Signal beruht auf der LTE Technik. Long Term Evolution oder kurz LTE ist und war vor allem dadurch erfolgreich, dass eine neue Übertragungstechnik verwendet wurde, mit der das Multipath-Fading-Effekt umgangen wurde. Der Multipath-Fading-Effekt tritt auf, wenn die Länge eines Übertragungsschrittes verkleinert wird, was bei einer Verzögerung des Signals dazu führen kann, dass sich die einzelnen Übertragungsschritte überlappen. Die Technik, die dem entgegen wirkt, hat die Bezeichnung Orthogonal Frequenz Division Multiplexing (OFDM) Technik. Hierbei wird ein schneller Datenstrom in viele kleine langsame unterteilt, da so kein Multipath-Fading-Effekt auftritt. Nach dem Übertragen wird der Datenstrom wieder zusammengeführt. OFDM wird in den folgenden Abschnitten noch weiter erläutert. Die Bandbreite von LTE ist recht variable wählbar und liegt zwischen \SI{1.25}{\mega\hertz} und \SI{20}{\mega\hertz}, in der bei diesem Projekt verwendeten Übertragungstechnik, die auf LTE basiert liegt die Bandbreite bei \SI{5}{\mega\hertz}. Die Wahl der Bandbreite hängt hier sehr davon ab, was das Ziel für die Applikation ist, theoretisch können aber bei einer Bandbreite von \SI{20}{\mega\hertz} und sehr guten Übertragungsbedingungen Datenraten von über \SI{100}{\mega\bit\per\second} erreicht werden. Weitere nennenswerte Änderungen waren die Einführung von Multiple Input Multiple Output (MIMO) Übertragungen sowie der Fokus auf das Paket vermittelnde Internet-Protokoll (IP)~\cite[S.~205f]{Sauter2018}.

\subsection{Aufbau von LTE}

Ein LTE-Signal besteht auf Übertragungsebene aus 504 unterschiedlichen Zellidentitäten. Diese sind in 168 unterschiedliche Gruppen unterteilt. Eine Zellidentität setzt sich zusammen aus Identifikationsnummer der Gruppe sowie der Identifikationsnummer der jeweiligen physikalischen Ebene in der physikalischen Gruppe zusammen. Somit berechnet sich die Zellidentität wie folgt:
\begin{equation}
    N_\text{ID}^\text{cell}=3N_\text{ID}^\text{(1)}+N_\text{ID}^\text{(2)}
\end{equation}
Hierbei gilt, dass jede Gruppe eine Nummer \(N_\text{ID}^{(1)}\) zur Identifikation im Bereich von \numrange{0}{167} besitzt. Die Identifikationsnummer \(N_\text{ID}^{(2)}\) der physikalischen Ebene in der physikalischen Gruppe im Bereich von \numrange{0}{2}.
Um dem Empfänger die Synchronisation auf das OFDM Signal zu ermöglichen, sind im LTE Signal zwei Synchronisierungssequenzen eingebettet. Diese werden als Primary Synchronisation Signal (PSS) und Secondary Synchronisation Signal (SSS) bezeichnet~\cite[S.~180]{etsi2021136}.

\subsubsection{Framestrukturtyp}

Der Framestrukturtyp 1, der für Vollduplex und Halbdublex FDD gilt, hat eine Länge von \SI{10}{\milli\second} und besteht aus \num{10} Subframes, die jeweils eine Länge von \SI{1}{\milli\second} haben und von \numrange{0}{9} durchnummeriert sind. Bei Subframes, die die Frequenz \(\Delta f=\SI{2.5}{\kilo\hertz}\), \(\Delta f=\SI{7.5}{\kilo\hertz}\) oder \(\Delta f=\SI{15}{\kilo\hertz}\) besitzen wird ein Subframe in zwei Slots unterteilt, wo die Länge jedes Slots \SI{0.5}{\milli\second} beträgt. Bei einer Subframefrequenz von \(\Delta f=\SI{1.25}{\kilo\hertz}\) besteht der Subframe aus keinen Slots und seine Länge beträgt \SI{1}{\milli\second}. Wenn die Übertragungsfrequenz bei \(\Delta f=\SI{0.37}{\kilo\hertz}\) liegt, besitzt ein Slot eine Länge von \SI{3}{\milli\second}. Eine Periode ist \SI{40}{\milli\second} lang und besteht dem nach aus \num{12} Slots, die von \numrange{0}{13} durchnummeriert sind. Die Übertragung beginnt somit bei jedem vierten Frame. Außerdem stehen für FDD \num{10} Subframes, \num{20} Slots oder bis \num{60} Subslots zur Verfügung. In Abbildung~\ref{fig:LTE_frame} ist zum besseren Verständnis der Frame mit Subframes und Slots dargestellt.

\begin{figure}
    \centering
    \begin{tikzpicture}
        \foreach \x in {0,...,2}
        \node [draw, thick, shape=rectangle, minimum width=2cm, minimum height=0.8cm, anchor=center] at (\x*2,0) {$\#\x$};
        \node [draw, thick, shape=rectangle, minimum width=2cm, minimum height=0.8cm, anchor=center] at (6,0) {$\dots$};
        \node [draw, thick, shape=rectangle, minimum width=2cm, minimum height=0.8cm, anchor=center] at (8,0) {$\#18$};
        \node [draw, thick, shape=rectangle, minimum width=2cm, minimum height=0.8cm, anchor=center] at (10,0) {$\#19$};
        \draw[thick,red,decorate,decoration={brace,amplitude=12pt}] (-1,0.4) -- (3,0.4) node[midway, above,yshift=12pt,]{Subframe};
        \draw[thick,green,decorate,decoration={brace,amplitude=12pt}] (-1,1.2) -- (11,1.2) node[midway, above,yshift=12pt,]{Frame};
        \draw[thick,blue,decorate,decoration={brace,amplitude=12pt}] (1,-0.4) -- (-1,-0.4) node[midway, below,yshift=-12pt,]{Slot};
    \end{tikzpicture}
...
PSS wird in jeden Frame zweimal übertragen, und zwar im ersten und \num{10} Slot. Innerhalb jedes Slots wird das PSS im letzten OFDM Symbol übertragen. Was UE damit erreicht mit dem PSS erreicht ist eine Subframe Synchronisation, eine Slot Synchronisation sowie eine Symbol Synchronisation. Außerdem kann die Mitte des jeweiligen Kanals bestimmt werden in der Frequenz Domain. Ebenfalls wird die passenden PCI aus den drei verschiedenen PCI erkannt.
Die Sequenz des PSS wird generiert durch die Zadoff-Chu erzeugt. Zadoff-Chu~\cite[S.~181]{etsi2021136}:
\begin{equation}
    \operatorname{d_u}(n)=\begin{cases} e^{-\mathrm{j}\frac{\pi un(n+1)}{63}}, & n=0,1,\dots,30 \\ e^{-\mathrm{j}\frac{\pi u(n+1)(n+2)}{63}}, & n=31,32,\dots,61 \end{cases}
\end{equation}
Bei einer MBMS-Zelle, die die Rahmenstruktur 1 verwendet, wird das Synchronsignal im Slot \num{0} in jedem vierten Subframe übertragen.

\subsubsection{Secondary Synchronisation Signal (SSS)}

Die für den sekundären Synchronisationssignal verwendete Sequenzen \(\operatorname{d}(0),\dots,\operatorname{d}(61)\) von zwei verknüpften Binärsequenzen der Länge \num{31}. Die verknüpften Sequenzen werden mit einer Verschlüsselungssequenz aus dem ersten Synchronisationssignal kodiert. Die Zuordnung der Sequenz zu den Resource-Elementen hängt von der Framestruktur ab. Bei einem Subframe des Framestrukturtyp 1 wird für das sekundäre Synchronisationssignal derselbe Antennenanschluss wie für das erstes verwendet~\cite[S.~183]{etsi2021136}.

\subsection{OFDM}

Wie schon thematisiert wird bei LTE die OFDM Technik verwendet, in diesem Unterkapitel wird nun auf die Technik genauer eingegangen. Nochmal zu Wiederholung durch das Orthogonales Frequenzmultiplexverfahren OFDM Technik ist es möglich mehr Daten gleichzeitig zu übertragen dafür wird das Signal aufgeteilt und auf unterschiedlichen Frequenzbändern (engl.\@ \textit{subcarriers}) übertragen.

Die Signalübertragung eines OFDM besteht aus drei Phasen: Preamble, Header und den Daten. Durch die Preamble wird Zeit Synchronisation, die Offset sowie die Kanalschätzung durchgeführt. Das Preamble besteht aus einer langen und kurzen Sequenz, die kurze ist zur Bestimmung der Zeit die lange ist für die restlichen Bestimmungen. Die kurze Sequenz besteht aus \num{10} periodischen Segmenten wo jedes die gleiche Abtastung von \num{16} Abtastungen besitzt. Die lange Sequenz besteht dagegen aus zwei OFDM-Symbolen. Hier folgen auf Schutzintervalle zwei FTT Intervalle.

Der Header eines OFDM besteht aus \num{5} Feldern der Datenrate (\SI{4}{\bit}), einem Reservierten \si{\bit} (\SI{1}{\bit}), der Länge (\SI{12}{\bit}), einem Parität Bit (\SI{1}{\bit}) und dem Füllbits (\SI{6}{\bit}). Beim Datenrate Feld kann hier zwischen \num{16} Kombinationen gewählt werden. Das Längenfeld gibt Information über die tatsächliche Länge in Bytes der Daten.

Das Datenformat besteht aus vier Felder Service, Nachricht, den Füllbits und Pad. Die Aufgabe des Pad-Feldes ist es, die Länge der Daten so anzupassen, dass die Datenlänge ist ein ganzzahliges Vielfaches von \(N\) dbps. Die gesamte Datenlänge ergibt sich aus der folgender Gleichung:
\begin{equation}
    N_\text{data}=N_\text{dbps}(Ceiling(\frac{16+8N_\text{Nachricht}}{N_\text{dbps}}))
\end{equation}
wobei \(N_\text{Nachricht}\) die Länge der Nachricht entspricht, die auch im Header übergeben wird. \num{16} entspricht der Anzahl an Service Bytes und \num{6} entspricht der Anzahl an Füllbits Bits. Die Anzahl an Pad hängt von der Daten Länge ab und berechnet sich wie folgt:
\begin{equation}
    N_\text{pad}=N_\text{data}-(16-8N_\text{Nachricht}+6)
\end{equation}
~\cite[S.~49ff]{Liu2019}
...
\begin{equation}\label{equ:cross_ambiguity_function}
    \psi(t, f_{d}) = \int_{-T/2}^{T/2} {x_{e}(s) \cdot x_{r}^{*}} \left( s - t \right)\mathrm{e}^{\mathrm{j} 2 \pi f_{d} s} \, \text{d} s
\end{equation}%
%
...
In der Praxis wird die CAF für ein ausgewähltes Intervall der Eingangsparameter \(m\) und \(k\) bzw.\@ \(t\) und \(f_d\) berechnet. Daraus resultiert eine Range-Doppler Map, in der nach potenziellen Zielen, sog.\@ Alarmen, gesucht werden kann. Die Einzelheiten solcher Detektionsalgorithmen, wie z.\,B.\@ dem weit verbreiteten Constant False Alarm Rate Detector (CFAR)~\cite[S.~208--230]{Malanowski2019}, sind nicht mehr Teil dieser Arbeit. Daher soll zunächst der Fokus auf der CAF bleiben.
...
\begin{description}
    ...
    \item[Fine Mode (CPU)]
        ...
        \begin{enumerate}
            \item Berechne das komplexe Produkt zwischen Echo und dem um \(\tau \) verzögerten Referenzsignal.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[
        ...
        \node [function node, anchor=east, left=0.25cm of dec0.west] (lpf0) {\(\mathbf{LPF} {\{ \cdot, \frac{1}{L} \}}\)};
        \node [function node, anchor=east, left=0.25cm of dec1.west] (lpf1) {\(\mathbf{LPF} {\{ \cdot, \frac{1}{L} \}}\)};
        \node [function node, anchor=east, left=0.25cm of dec2.west] (lpf2) {\(\mathbf{LPF} {\{ \cdot, \frac{1}{L} \}}\)};
        \node [function node, anchor=east, left=0.25cm of decD.west] (lpfD) {\(\mathbf{LPF} {\{ \cdot, \frac{1}{L} \}}\)};
        \node [function node, anchor=east, left=0.25cm of decN.west] (lpfN) {\(\mathbf{LPF} {\{ \cdot, \frac{1}{L} \}}\)};
